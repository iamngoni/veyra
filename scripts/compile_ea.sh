...
#
# The versioned source keeps __VEYRA_URL__ and __VEYRA_TOKEN__ placeholders;
# both are injected from the environment at compile time, so no secret or
# environment hostname is committed. VEYRA_EA_URL defaults to the loopback
# endpoint; set it to the tunnel URL (https://veyra.antonlabs.cc/ea/poll) when
# the terminal reaches Veyra through Cloudflare instead of localhost.
