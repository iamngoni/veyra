...
- `close_order` and `modify_order` shipped on the same pattern: the service
  accepts only tickets from the latest completed `account_snapshot` carrying
  the Veyra magic, and the terminal re-checks the magic before closing or
  changing stops.
