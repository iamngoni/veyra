...
| Venue contracts (`symbol_spec`) | Live-proven against IFC Markets: spread, stop level, lot band, margin per lot, and swap rates for every menu symbol |
...
| Diagnostics/control listener | `/health`, `/ready`, `/status`, `/metrics`, `/intents/*`, `/commands*`, `/events`, `/logs`, `/audit`, `/account`, `/market/candles`, `/market/spec`, `/reconciliation` | `127.0.0.1:8080` (`VEYRA_BIND_HOST`/`VEYRA_BIND_PORT`) |
...
3. **Instrument contracts.** Before the entry decision, `MarketFeed::symbol_spec` queues a read-only `symbol_spec` command per candidate so the model sees the venue's own contract; a candidate whose contract is unavailable is still reviewed but its entries are rejected as unverifiable rather than queued blind.
...
9. **Venue contract check (pre-queue).** The approved draft is measured against the instrument contract: volume lands on the venue's lot band and step, estimated margin (`marginRequired x lots`) fits the reported free margin, and the stop sits beyond both the current spread and the broker's minimum stop level. Violations are audited as `rejected` with stable codes and nothing is queued.
